...
- **주요 서비스**: us-app-prod (Flutter RUM), us-insight-api-prod, us-campus (백엔드), MongoDB Atlas (cluster0)
...
- **파라미터**:
  - `query`: Datadog 메트릭 쿼리 문자열 (예: `avg:mongodb.atlas.system.disk.iops.reads{host:cluster0-shard-00-03.k5vvm.mongodb.net}`)
  - `from`: 시작 시간 (epoch seconds 또는 `now-1h` 같은 상대 시간)
  - `to`: 종료 시간
- **사용 예시**:
  - MongoDB shard별 디스크 I/O, 메모리, CPU 조회
...
모바일 앱(us-app-prod) 사용자 세션, 에러, 성능 데이터 조회.
...
```json
{
  "mcpServers": {
    "datadog": {
      "command": "node",
      "args": ["dist/index.js"],
      "cwd": "/Users/thkim/Documents/git/datadog-mcp-server",
      "env": {
        "DD_API_KEY": "<api-key>",
        "DD_APP_KEY": "<app-key>",
        "DD_SITE": "us5.datadoghq.com"
