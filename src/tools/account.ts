...
export const getUsageSummarySchema = z.object({
  startMonth: z.string().describe("Start month as ISO 8601 date (e.g., 2024-01-01T00:00:00Z)"),
  endMonth: z.string().optional().describe("End month as ISO 8601 date"),
  includeOrgDetails: z.boolean().optional().describe("Include organization details"),
});
...
export const listUsersSchema = z.object({
  pageSize: z.number().optional().default(50).describe("Number of results per page"),
  pageNumber: z.number().optional().default(0).describe("Page number (0-based)"),
  filter: z.string().optional().describe("Search filter string"),
  filterStatus: z.string().optional().describe("Filter by user status (Active, Pending, Disabled)"),
  sort: z.string().optional().describe("Sort field (name, email, created_at)"),
